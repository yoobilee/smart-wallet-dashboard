...
> ⚠️ CODEF 데모 서비스는 최초 1회, 1개월만 이용 가능합니다. 데모 기간이 만료되어 현재 배포
> 버전은 실제 API 대신 동일한 응답 형식의 Mock 데이터로 동작합니다. 실제 신한은행/현대카드
> 연동 로직은 `server/src/codef.js`에 그대로 남아있으며, 환경변수(`USE_MOCK_CODEF`) 하나로
> 실제 연동과 Mock 응답을 전환할 수 있도록 구현했습니다.
